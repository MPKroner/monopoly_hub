...
<body data-theme="dark">
  ...
  <script>
    ...
    const STORAGE_KEY = "monopoly_collection_db_v2";
    ...
    function generateVectorBoxHtml(item) {
      ...
      const title = item.name.replace(/^Monopoly\s*(:|-)?\s*/i, '');
    ...
    function renderCards(items, container) {
      ...
      items.forEach(item => {
        ...
        if (item.image_url) {
          coverHtml = 
            '<img class="box-cover-image" src="' + item.image_url + '" alt="' + item.name + '" onerror="this.style.display=\'none\'; this.nextElementSibling.style.display=\'flex\';">' +
    ...
    function renderTable(items, tbody) {
      ...
      items.forEach(item => {
        ...
        let thumbHtml = "";
        if (item.image_url) {
          thumbHtml = '<img class="table-thumb" src="' + item.image_url + '" alt="Boîte" data-lightbox-id="' + item.id + '" onerror="this.style.display=\'none\';">';
        } else {
    ...
    // IMAGE FILE UPLOAD HELPERS (converts file to data URL)
    function handleImageUpload(file, inputTargetId, previewContainerId, currentItem) {
      if (!file) return;
      const reader = new FileReader();
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      // Global delegation: Click on Lightbox, Status Toggle, Edit
      ...
      document.getElementById("editImageFileInput").addEventListener("change", (e) => {
        const id = document.getElementById("editItemId").value;
        const item = database.find(e => e.id === id);
        handleImageUpload(e.target.files[0], "editImageUrl", "modalImagePreviewContainer", item);
      });
      ...
      document.getElementById("lightboxEditBtn").addEventListener("click", () => {
        document.getElementById("lightboxModalBackdrop").classList.remove("open");
      ...
      document.getElementById("addImageFileInput").addEventListener("change", (e) => {
        handleImageUpload(e.target.files[0], "addImageUrl", "addModalImagePreview", null);
      });
...
print("Updated index.html with 3D box covers and image support successfully written!")
